        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-10 md:mb-14">
          <div>
            <div className="flex items-center gap-3 mb-3">
              <span className="text-xs font-bold uppercase tracking-widest" style={{ color: '#166b7f' }}>Quality Certifications</span>
          ...
          <div className="shrink-0 border border-white/10 bg-white/5 px-5 py-4 flex items-center gap-3 self-start sm:self-auto">
            <Award size={20} style={{ color: '#166b7f' }} />
            <div>
          ...
          <div className="flex-1 relative" aria-live="polite">
            ...
            >
              {cert.image?.url ? (
                ...
                <div className="relative h-56 sm:h-72 lg:h-80 bg-[#0a1121]">
                  <img
                    ...
                    style={{ filter: 'drop-shadow(0 0 24px rgba(22,107,127,0.08))' }}
                  />
                  <div className="absolute inset-0 pointer-events-none"
                ...
                <div className="relative flex flex-col items-center justify-center px-8 py-14 sm:py-20 text-center"
                  style={{ background: 'linear-gradient(135deg,rgba(22,107,127,0.04) 0%,rgba(0,105,111,0.06) 100%)' }}>
                  {/* Decorative border corners */}
                  <div className="absolute top-4 left-4 w-8 h-8 border-t border-l" style={{ borderColor: 'rgba(22,107,127,0.3)' }} />
                  <div className="absolute top-4 right-4 w-8 h-8 border-t border-r" style={{ borderColor: 'rgba(22,107,127,0.3)' }} />
                  <div className="absolute bottom-4 left-4 w-8 h-8 border-b border-l" style={{ borderColor: 'rgba(22,107,127,0.3)' }} />
                  <div className="absolute bottom-4 right-4 w-8 h-8 border-b border-r" style={{ borderColor: 'rgba(22,107,127,0.3)' }} />

                  <div className="inline-flex h-16 w-16 items-center justify-center border border-[#166b7f]/30 bg-[#166b7f]/5 mb-5">
                    <ShieldCheck size={32} style={{ color: '#166b7f' }} />
                  </div>
                  ...
                  <div className="mt-5 px-4 py-1.5 text-xs font-bold uppercase tracking-wider font-['JetBrains_Mono']"
                    style={{ color: '#166b7f', border: '1px solid rgba(22,107,127,0.25)', background: 'rgba(22,107,127,0.06)' }}>
                    Certified
                  </div>
                </div>
              )}
              ...
              <div className="flex items-center gap-3 px-5 py-4 border-t border-white/10">
                <ShieldCheck size={15} style={{ color: '#166b7f', flexShrink: 0 }} />
          ...
          <div className="lg:w-56 shrink-0 flex flex-row lg:flex-col gap-2 overflow-x-auto lg:overflow-visible">
            {certificates.map((c, index) => (
              <button
                ...
                style={{
                  background: index === activeIndex ? 'rgba(22,107,127,0.08)' : 'rgba(255,255,255,0.03)',
                  borderColor: index === activeIndex ? 'rgba(22,107,127,0.4)' : 'rgba(255,255,255,0.08)',
                  minWidth: 160,
                }}
              >
                <div className="w-1.5 h-1.5 rounded-full shrink-0 transition-colors"
                  style={{ background: index === activeIndex ? '#166b7f' : 'rgba(255,255,255,0.2)' }} />
        ...
        {certificates.length > 1 && (
          <div className="flex items-center gap-4 mt-5">
            <button type="button" onClick={showPrevious} aria-label="Previous certificate"
              className="inline-flex items-center justify-center w-9 h-9 border border-white/15 text-white hover:border-[#166b7f] hover:text-[#166b7f] transition-colors">
              <ChevronLeft size={18} />
            </button>
              ...
              {certificates.map((c, index) => (
                <button key={c._id || index} type="button" onClick={() => setActiveIndex(index)}
                  ...
                  style={{
                    ...
                    height: 4,
                    background: index === activeIndex ? '#166b7f' : 'rgba(255,255,255,0.2)',
                    borderRadius: 2,
                    border: 'none',
                    padding: 0,
                    cursor: 'pointer',
