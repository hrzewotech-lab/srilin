...
const fallbackSlides = [
  { _id: 'fallback-hero-1', title: 'Precision Electronics Manufacturing', description: 'Certified electronics design and manufacturing support for prototype, SMT assembly, testing, and box build programs.', image: { url: '/header1.webp' }, tag: 'AS9100D Certified EMS' },
  { _id: 'fallback-hero-2', title: 'High-Reliability EMS for Demanding Products', description: 'Aerospace-aware quality systems, ESD controls, and disciplined inspection workflows from Hyderabad.', image: { url: '/header2-2.webp' }, tag: 'ISO 9001:2015 Certified' },
  { _id: 'fallback-hero-3', title: 'From Embedded Design to Final Integration', description: 'A practical one-stop manufacturing partner for teams scaling complex electronic products with confidence.', image: { url: '/header3-2.webp' }, tag: 'ANSI ESD S20.20 2021' },
];
...
export default function HeroCarousel() {
  ...
  useEffect(() => {
    const loadSlides = async () => {
      try {
        ...
        const activeSlides = (res.data.slides || []).filter((s) => s.image?.url);
        setSlides(activeSlides.length ? activeSlides : fallbackSlides);
      } catch (error) { console.error('Failed to load hero slides', error); setSlides(fallbackSlides); }
    };
    loadSlides();
  }, []);
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => setActiveIndex((p) => (p + 1) % slides.length), 5500);
  ...
  return (
    ...
    >
      <div aria-live="polite">
        {slides.map((slide, index) => (
          <div key={slide._id || index} className="absolute inset-0"
            style={{ opacity: index === activeIndex ? 1 : 0, zIndex: index === activeIndex ? 1 : 0, transition: 'opacity 1.1s ease' }}>
            <img src={slide.image?.url} alt={slide.title || `Featured image ${index + 1}`}
              className="w-full h-full object-cover object-center"
              style={{ filter: 'brightness(0.36)', transform: index === activeIndex ? 'scale(1.06)' : 'scale(1)', transition: 'transform 6s ease' }} />
          </div>
        ))}
      </div>
      ...
      <div className="relative flex flex-col justify-end h-full max-w-7xl mx-auto px-4 sm:px-8 md:px-12 pb-20 sm:pb-28" style={{ zIndex: 10 }}>
        <div style={{ animation: 'heroSlideUp 0.75s cubic-bezier(0.16,1,0.3,1) both', maxWidth: 680 }}>
          ...
          <p className="leading-relaxed mb-8"
            style={{ fontSize: 'clamp(0.9rem,1.8vw,1.1rem)', color: 'rgba(203,213,225,0.85)', maxWidth: 520, opacity: typeDone ? 1 : 0, transform: typeDone ? 'none' : 'translateY(8px)', transition: 'opacity 0.6s ease, transform 0.6s ease' }}>
          ...
          <div className="flex flex-wrap gap-3" style={{ opacity: typeDone ? 1 : 0, transform: typeDone ? 'none' : 'translateY(10px)', transition: 'opacity 0.6s 0.15s ease, transform 0.6s 0.15s ease' }}>
            ...
            <a href="/services" className="inline-flex items-center gap-2 text-sm font-semibold px-7 py-3.5 text-white transition-colors hover:bg-white/10" style={{ border: '1px solid rgba(255,255,255,0.22)' }}>
              Explore Services
            </a>
          </div>
        </div>
      ...
      <div className="hidden xl:flex flex-col gap-2 absolute right-10 bottom-24" style={{ zIndex: 10 }}>
        {heroStats.map(({ value, label }, i) => (
          <div key={label} className="flex items-center gap-4 px-5 py-3 border border-white/10"
            style={{ background: 'rgba(15,23,42,0.72)', backdropFilter: 'blur(10px)', animation: `heroStatIn 0.5s ${0.4 + i * 0.12}s ease both` }}>
      ...
      {slides.length > 0 && (
        <div className="absolute right-5 sm:right-10 bottom-8 font-['JetBrains_Mono'] text-xs tracking-widest" style={{ zIndex: 10, color: 'rgba(255,255,255,0.38)' }}>{slideLabel}</div>
      )}
