...
export default function DesignEngineeringPage() {
  ...
  const iconMap = {
    Cpu, Pencil, Microscope, FlaskConical, MonitorDot, Layers, Zap
  };

  const heroText = content?.design_engineering_hero_title || 'Embedded Systems. Built to Last.';
  const sections = content?.design_engineering_sections || [];
  ...
  return (
    <div className="bg-[#f7f9fb] font-['Inter'] min-h-screen">
      ...
      <div className="max-w-6xl mx-auto px-6 md:px-12 py-16 md:py-20 space-y-6">
        ...
        {sections.map(({ title, eyebrow, icon, tag, image, alt, body, bullets }, index) => {
          const isEven = index % 2 === 0;
          const hasImage = !!image;
          const Icon = typeof icon === 'string' ? (iconMap[icon] || CheckCircle2) : (icon || CheckCircle2);
