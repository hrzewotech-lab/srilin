...
exports.getAllContent = async (req, res) => {
  try {
    ...
    const content = await SiteContent.find(filter).sort({ page: 1, key: 1 });
...
exports.updateContent = async (req, res) => {
  try {
    ...
    if (content) {
      content.value = value !== undefined ? value : content.value;
...
// Seed initial content (dev utility)
exports.seedContent = async (req, res) => {
  try {
    let seedData = req.body.seedData;
    
    // If no seedData provided from frontend, use default hardcoded data
    ...
    for (const item of seedData) {
      ...
      if (!existing) {
        const newDoc = await SiteContent.create(item);
        created.push(newDoc);
      }
    }
    
    res.status(200).json({ success: true, message: `Seeded ${created.length} new items`, created });
  } catch (error) {
